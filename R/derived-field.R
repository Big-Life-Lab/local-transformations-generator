...
derived_field.get_pmml_str_for_var <- function(var_name, expr, tokens, comment_tokens, evaluated_variables) {
  ...
  get_pmml_str_for_if_expr <- function(cond_expr_to_block_exprs_mappings) {
    ...
    for(i in 1:length(cond_expr_to_block_exprs_mappings)) {
      ...
      for(j in 1:length(cur_mapping$block_expr_ids)) {
        cur_expr_token <- getTokenWithId(cur_mapping$block_expr_ids[[j]], tokens)
        var_names_in_cur_expr <- util.get_var_and_func_names(getDescendantsOfToken(cur_expr_token, tokens))
    ...
    for(i in 1:length(reverse_mappings)) {
      ...
      if(is.na(cur_mapping$cond_expr_id) == FALSE) {
        pmml_str_for_cond <- get_pmml_str_for_expr(
          getTokenWithId(cur_mapping$cond_expr_id, tokens),
          tokens
        )
      }
      pmml_str_for_expr <- get_pmml_str_for_token(
        getTokenWithAssignmentCode(
          getDescendantsOfToken(getTokenWithId(cur_mapping$expr_id, tokens), tokens)
        ),
        tokens,
        tokens.create_empty_tokens_df(),
        evaluated_variables
      )
  ...
  get_pmml_str_for_expr <- expr.generic_get_pmml_str_for_expr(get_pmml_str_for_row_access, get_pmml_str_for_func_call_row_access, get_pmml_str_for_if_expr)
  get_pmml_str_for_token <- pmml.generic_get_pmml_str_for_token(get_pmml_str_for_expr)
  
  if(if_expr.is(expr, tokens)) {
